...
export class StubAdapter extends BasePeerAdapter implements PeerAdapter {
  ...
  async probe(): Promise<PeerProbeResult> {
    return {
      ...
      message: "Stub enabled by CROSS_REVIEW_SDK_STUB=1.",
    };
  }
  ...
  async call(prompt: string, context: PeerCallContext): Promise<PeerResult> {
    ...
    return this.resultFromText({
      ...
      started: Date.now(),
      attempts: 1,
      modelReported: process.env.CROSS_REVIEW_SDK_STUB_REPORTED_MODEL,
    });
  }
  ...
  async generate(prompt: string, context: PeerCallContext): Promise<GenerationResult> {
    ...
    const text = [
      "# Test Draft",
      "",
      "This text was generated by the stub only because CROSS_REVIEW_SDK_STUB=1 is active.",
      "",
      prompt.slice(0, 1200),
    ].join("\n");
